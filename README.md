...
1. **Исключение папки проекта в антивирусе** — иначе он сканирует каждый из ~40 000 файлов внутри
   `node_modules` при установке и сборке. Если у тебя сторонний AV (Kaspersky, Dr.Web, ESET и т.п.),
   Defender переводится в пассивный режим, и `Add-MpPreference` ничего не ускорит — исключения нужно
   добавлять в сам антивирус («Доверенная зона» / «Исключения» / «Не отслеживать файлы»).
   Если стороннего AV нет — от админа:
   ```powershell
   Add-MpPreference -ExclusionPath "C:\путь\к\rubex"
   ```
   Проверить активный AV: `Get-CimInstance -Namespace root/SecurityCenter2 -ClassName AntiVirusProduct`.
...
4. **`npm ci` вместо `npm install`** (ставит ровно из lock-файла, быстрее и без разрешения зависимостей),
   и держи `node_modules` на SSD, а папку проекта — вне OneDrive/«Загрузок» с включённым-sync:
   синхронизация пересканирует каждую переустановку.
